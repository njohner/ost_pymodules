...
This example represents a typical script used to calculate the membrane elastic properties from a coarse-grained Martini simulation.
As input you need an aligned trajectory (see the align_trajectory_Martini.py for an example of how to align a trajectory).
...
#    For example if the system contains DOP (DOPC) and CHO (cholesterol).
lipid_names=['CHO','DOP']
...
to_analyze="cname=A and rname=DOP,CHO"
to_ignore="cname!=A and rname=DOP,CHO"

#6.  We need to define the area per lipid (at the neutral plane). This is used in the
#    Calculation of the lipid splay constants.
#    In this example it is set to 49.5 A^2. For a lipid bilayer the area can be obtained
